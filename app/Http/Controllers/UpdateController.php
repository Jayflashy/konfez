        ...
        if ($request->has('update_file')) {
            if (class_exists('ZipArchive')) {
                ...
                if ($res === true) {
                    ...
                    return back()->withError('Could not open the updates zip file.');
                }

                return redirect()->route('update.finish');
            }
            else {
                return back()->withError('Please enable ZipArchive extension.');
            }
        }
        else {
            return view('update.index');
    ...
    public function saveDatabase($request){        
        ...
        if (is_file($path)) {
            try {
                ...
                return back()->withError('Could not open the updates sql file. Try again');
            }
            unlink(base_path('/update.sql')); 
        }
        return redirect()->route('update.finish');
